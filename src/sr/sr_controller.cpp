...
void Configuration::save() const {
    ...
    s.setValue("sr/memoryMiB",memoryMiB); s.setValue("sr/halo",halo);
}
...
Controller::Controller(MainWindow* window,QVGraphicsView* view)
    ...
    status_=new QLabel(QStringLiteral("JPEG/PNGを開いてください"),toolbar_); status_->setObjectName("srStatus");
    status_->setMargin(6); status_->setMinimumWidth(150); status_->setMaximumWidth(440);
    status_->setWordWrap(true); status_->setTextInteractionFlags(Qt::TextSelectableByMouse);
    ...
    connect(&watchdog_,&QTimer::timeout,this,[this] {
        ...
        if(job_->process) job_->process->kill(); else finish(job_,job_->error);
    });
...
Controller::~Controller() {
    ...
    if(job_) {
        job_->cancelled=true;
        if(job_->process) {
            // Shutdown must not publish results or touch a window being destroyed.
            job_->process->disconnect(this);
            job_->process->kill(); job_->process->waitForFinished(2000);
        }
    }
}
...
void Controller::sourceLoaded() {
    sourceReady_=!view_->getImageCore().getSourceImage().isNull() && !view_->getCurrentFileDetails().isMovieLoaded;
    if(!job_) {
        status_->setText(sourceReady_?QStringLiteral("元画像 · ")+view_->getImageCore().getSourceProfile().description:
                         QStringLiteral("静止JPEG/PNGを開いてください"));
        if(sourceReady_ && !view_->getImageCore().getSourceProfile().error.isEmpty()) status_->setText(view_->getImageCore().getSourceProfile().error);
    }
    if(sourceReady_ && view_->getImageCore().getSourceProfile().error.isEmpty()) display();
    updateActions();
}
...
void Controller::display() {
    ...
    if(destination.isEmpty()) { status_->setText(QStringLiteral("画面ICCを読み込めません。SR設定を確認してください。")); return; }
    ...
    if(image.isNull()) { status_->setText(readFailure(error)); return; }
    ...
    if(!job_) status_->setText(showingSr_?QStringLiteral("SR ×4 · %1×%2").arg(result_.width()).arg(result_.height()):
...
void Controller::start() {
    ...
    if(!QFileInfo(config.workerPath).isExecutable() || !QFileInfo::exists(config.modelPath) ||
       !QFileInfo::exists(config.runtimeRoot+"/deployment_tools/inference_engine/lib/intel64/libmyriadPlugin.so")) {
        status_->setText(QStringLiteral("worker / モデル / runtimeが見つかりません。SR設定を確認してください。"));
    ...
    if(pixels*224 + 4*192ULL*1024*1024 > quint64(config.memoryMiB)*1024*1024) {
        status_->setText(QStringLiteral("SR用メモリー上限を超えます。小さい画像またはSR設定の上限を選んでください。"));
        emit failed(status_->text()); return;
    }
    ...
    if(!job->directory->isValid()) { status_->setText(QStringLiteral("一時フォルダーを作成できません")); return; }
    job_=job; status_->setText(QStringLiteral("sRGB入力を準備中…"));
...
void Controller::launch(const std::shared_ptr<Job>& job) {
    ...
    QJsonObject request{{"protocol_version",1},{"job_id",job->id},{"source_key",job->sourceKey},
        ...
        {"halo",job->configuration.halo},{"devices",job->configuration.devices},
        {"max_memory_bytes",double(job->configuration.memoryMiB)*1024*1024},{"lock_file",runtimeDir+"/qviewsr-devices.lock"}};
    QFile file(job->directory->filePath("job.json"));
    const auto json=QJsonDocument(request).toJson();
    if(!file.open(QIODevice::WriteOnly) || file.write(json)!=json.size()) { finish(job,QStringLiteral("jobを保存できません")); return; }
    file.close();
    auto* process=new QProcess(this); job->process=process;
    process->setProcessEnvironment(job->configuration.environment());
    connect(process,&QProcess::readyReadStandardOutput,this,[this,job]{readEvents(job);});
    connect(process,&QProcess::readyReadStandardError,this,[job] {
        job->log+=job->process->readAllStandardError();
        if(job->log.size()>65536) job->log=job->log.right(65536);
    });
    connect(process,&QProcess::errorOccurred,this,[this,job](QProcess::ProcessError error) {
        if(error==QProcess::FailedToStart) { const auto message=job->process->errorString(); job->process->deleteLater(); finish(job,message); }
    });
    connect(process,qOverload<int,QProcess::ExitStatus>(&QProcess::finished),this,[this,job,process](int code,QProcess::ExitStatus exitStatus) {
        readEvents(job); watchdog_.stop(); process->deleteLater();
        if(job!=job_) return;
        if(job->cancelled || job->generation!=generation_) { finish(job); return; }
        if(code!=0 || exitStatus!=QProcess::NormalExit || job->completion.isEmpty() || !job->error.isEmpty()) {
            finish(job,job->error.isEmpty()?QStringLiteral("workerが終了しました: ")+QString::fromUtf8(job->log.right(1000)):job->error); return;
        }
        status_->setText(QStringLiteral("SR画像を確認中…"));
        ...
        connect(watcher,&QFutureWatcher<QImage>::finished,this,[this,job,watcher] {
            ...
            for(const auto& value:job->completion.value("devices").toArray()) {
                auto device=value.toObject(); stats<<QStringLiteral("%1: %2 tiles").arg(device.value("id").toString()).arg(device.value("tiles").toInt());
            }
            resultSummary_=stats.join(" / ");
        ...
        watcher->setFuture(QtConcurrent::run([job] {
            ...
            if(completion.value("source_key").toString()!=job->sourceKey || completion.value("color_space").toString()!="sRGB" ||
               completion.value("width").toInt()!=expected.width() || completion.value("height").toInt()!=expected.height())
                return fail(QStringLiteral("workerの結果情報が一致しません"));
    ...
    status_->setText(QStringLiteral("デバイスを起動中…")); watchdog_.start(120000);
    process->start(job->configuration.workerPath,{"--job",file.fileName()});
}
void Controller::readEvents(const std::shared_ptr<Job>& job) {
    if(!job->process) return;
    job->buffer+=job->process->readAllStandardOutput();
    while(job->buffer.contains('\n')) {
        const auto newline=job->buffer.indexOf('\n');
        const auto line=job->buffer.left(newline); job->buffer.remove(0,newline+1);
        QJsonParseError error;
        const auto message=QJsonDocument::fromJson(line,&error).object();
        if(line.size()>65536 || error.error!=QJsonParseError::NoError || message.value("protocol_version").toInt()!=1 || message.value("job_id").toString()!=job->id) {
            job->error=QStringLiteral("worker通信形式が不正です"); job->process->kill(); return;
        }
        if(job!=job_ || job->cancelled) continue;
        const auto event=message.value("event").toString();
        if(event=="error") job->error=message.value("message").toString();
        else if(event=="completed") job->completion=message;
        else if(event=="device_error") job->warnings<<message.value("device").toString()+": "+message.value("message").toString();
        else if(event=="device_ready") status_->setText(QStringLiteral("準備完了: ")+message.value("device").toString());
        else if(event=="progress") {
            const int total=message.value("total").toInt(),done=message.value("completed").toInt();
            if(total<1 || done<0 || done>total) { job->error=QStringLiteral("進捗情報が不正です"); job->process->kill(); return; }
            progress_->setRange(0,total); progress_->setValue(done);
            status_->setText(QStringLiteral("処理中 %1/%2 · %3").arg(done).arg(total).arg(message.value("device").toString()));
        }
        watchdog_.start(120000);
    }
    if(job->buffer.size()>65536) { job->error=QStringLiteral("worker通信が上限を超えました"); job->process->kill(); }
}
void Controller::finish(std::shared_ptr<Job> job,const QString& error) {
    ...
    if(!error.isEmpty()) { status_->setText(readFailure(error)); status_->setToolTip(QString::fromUtf8(job->log)); emit failed(error); }
    else if(cancelled) status_->setText(sourceReady_?QStringLiteral("中止しました · 元画像"):QStringLiteral("画像を読み込み中…"));
    updateActions();
}
void Controller::cancel() {
    if(!job_) return;
    job_->cancelled=true; watchdog_.stop(); status_->setText(QStringLiteral("中止中…")); updateActions();
    const QPointer<QProcess> process=job_->process;
    if(process && process->state()!=QProcess::NotRunning) {
        process->terminate();
        QTimer::singleShot(2000,this,[process] { if(process && process->state()!=QProcess::NotRunning) process->kill(); });
    }
}
...
void Controller::saveAs() {
    ...
    if(QFileInfo(path).absoluteFilePath()==original.absoluteFilePath() ||
       (!QFileInfo(path).canonicalFilePath().isEmpty() && QFileInfo(path).canonicalFilePath()==original.canonicalFilePath())) {
        status_->setText(QStringLiteral("元画像への上書きはできません")); return;
    }
    ...
    auto* watcher=new QFutureWatcher<QString>(this);
    saving_=true; updateActions(); status_->setText(QStringLiteral("SR画像を保存中…"));
    connect(watcher,&QFutureWatcher<QString>::finished,this,[this,watcher,generation,path] {
        ...
        if(generation==generation_) status_->setText(error.isEmpty()?QStringLiteral("保存しました: ")+QFileInfo(path).fileName():error);
    });
...
void Controller::showSettings() {
    ...
    auto* runtime=field(QStringLiteral("OpenVINO runtime"),configuration_.runtimeRoot,true);
    auto* worker=field(QStringLiteral("worker"),configuration_.workerPath,false);
    auto* model=field(QStringLiteral("1032 FP16 XML"),configuration_.modelPath,false);
    ...
    auto* scanResult=new QLabel(&dialog); scanResult->setWordWrap(true); scanResult->setTextInteractionFlags(Qt::TextSelectableByMouse); layout->addWidget(scanResult);
    connect(scan,&QPushButton::clicked,&dialog,[&,scanResult] {
        ...
        connect(process,qOverload<int,QProcess::ExitStatus>(&QProcess::finished),&dialog,[=](int,QProcess::ExitStatus) {
            ...
            for(const auto& line:lines) {
                ...
                for(const auto& id:object.value("devices").toArray()) ids<<id.toString();
    ...
    auto* halo=new QSpinBox(&dialog); halo->setRange(0,64); halo->setValue(configuration_.halo); form->addRow(QStringLiteral("境界の余白（LR pixel）"),halo);
    auto* note=new QLabel(QStringLiteral("SRは×4・sRGBで処理/保存します。画面ICCは保存画像に適用しません。\n個体を指定する場合はIDをカンマ区切りで入力できます。世代混在時の画質は比較検証中です。"),&dialog);
    ...
    conf.memoryMiB=memory->value(); conf.halo=halo->value(); setConfiguration(conf);
}
}
