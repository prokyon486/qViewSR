...
| Ctrl＋→／Ctrl＋← | 表示を右（時計回り）／左（反時計回り）に1度ずつロール。カメラ位置・注視点・距離・画角は一定 |
| 左ドラッグ | カメラの平行移動 |
| ホイール | カメラが近づく／遠ざかる。画角は一定 |
| Shift＋ホイール | 画角を変更（5～120度）。カメラ位置は一定 |
| 中央クリック／「3Dの全体表示に戻す」／既存の表示リセット | モデル回転・カメラのロール・平行移動・画角をリセットして全体表示 |
...
PNGは**背景を透明**にし、sRGBプロファイル付きで保存します。モデルの隙間、透過テクスチャ、半透明材質、
輪郭のアンチエイリアスも描画のアルファ値を保ちます。背景色と同じ色のモデル部分も保持します。
画面上は引き続き暗い背景で表示し、保存後も表示を維持します。GLB保存や3Dへの超解像は行いません。
ロール操作は「3D表示」メニューからも実行できます。ロール後もドラッグの方向は画面に合わせます。
...
- オリジナルの簡易スタジオ環境光と2灯の補助照明を使用します。他の3Dソフトの照明・描画とは見え方が異なります。
  環境光と補助照明はワールド座標に固定し、モデル回転やカメラのロールには追従しません。
  モデルを回して照明の当たり方を変え、カメラ操作で構図を調整できます。
...
GPUテストには内部で生成する、原点から離れたテクスチャ付きキューブと、穴・半透明部分のあるカードも使います。
透明PNGの背景・モデル・輪郭・材質のアルファ値、画面背景への再合成、保存失敗時も含む表示復元を確認します。
Ctrl＋矢印の1度ずつのロール、位置・注視方向・画角の不変、通常の矢印キーによるファイル移動も検証します。
CIは実行せず、pushするコミットには`[skip ci]`を付けます。

2026-09-26のローカル検証では、Ubuntu 24.04・Qt 6.4.2のデスクトップ上で、
ユーザー提供の28個（約42～54 MB）のGLBすべての表示・回転・透明PNG保存と元ファイルのハッシュ不変を確認しました。
28枚のPNGについてRGBA形式・透明背景・不透明なモデル部分・輪郭の中間アルファ・ICCプロファイルを確認しています。
通常表示とHiDPI 2倍でカメラのロール、透過材質、保存後の表示復元、保存寸法、全画面、画像との往復、
NCS workerの起動抑止と再利用を検証しています。
...
[SceneEnvironmentの背景モード](https://doc.qt.io/qt-6.5/qml-qtquick3d-sceneenvironment.html#backgroundMode-prop)、
