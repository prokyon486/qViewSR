...
- WebP、BMP、TIFF、PPM、ICO、XPM、SVGの読込からSR。当時のGIFは現在フレームのみを処理（下記の全フレーム対応で置換済み）。CMYK JPEGはdecoder RGB/sRGB仮定経路を確認。
...
### 倍率指定・重ね処理の実機確認（2026-09-22）

`realScaleAndRepeatGui`をX11上で実行し、NCS×2＋NCS2×2で成功。897×477の入力を2倍（1794×954）にした後、そのSR結果へ3倍処理を行い、5382×2862（累計6倍）を表示・PNG保存した。2処理は同一worker PIDで、初期化は1回のみ。元画像ボタンによる897×477への復帰と、無加工のsource QImageの保持も確認した。

workerの単発実測は初回9タイル4.30秒、追加25タイル8.77秒（初期化・GUI側の最終縮小時間を除く）。モデルは両方とも4倍推論で、GUI側が2倍／3倍に縮小する。常に画質が改善するという評価ではない。

画面と保存結果はローカルの`diagnostics/local/gui-scale-check/`、実機テストのログは`diagnostics/local/scales-hardware.log`に保存。メッセージ領域がウィンドウ右端まで伸び、累計倍率・処理回数・寸法を1行表示することを画面でも確認した。


### GIF全フレームSR・再生の実機確認（2026-09-22）

`realAnimationGui`をX11上で実行し、NCS×2＋NCS2×2で成功。自作の500×280・3フレームのGIF（100 / 200 / 300 ms、無限ループ）を全フレーム2倍の1000×560にし、さらに全フレームを2倍の2000×1120へ処理した。6フレームの推論は同一worker PID・初期化1回で完了。再生、停止、コマ送り、同じ位置での元画像切替、各フレームのPNG保存を検証した。

| 処理 | 全フレームのタイル数 | worker処理時間合計（初期化除く） |
|---|---|---|
| 元GIF全3フレームを2倍 | 12（各個体3） | 6.38秒 |
| 上記結果の全3フレームへさらに2倍 | 27（NCS2が8/7、NCSが6/6） | 12.80秒 |

上記は単発の機能試験値。フレーム間隔は元の100 / 200 / 300 msを保持し、処理時間を再生時間として使わない。画面・保存画像・統計は`diagnostics/local/gui-gif-check/`、実機ログは`diagnostics/local/gif-hardware.log`に保存した。

ローカルのGUI試験はActionManagerTests・SrTestsとも合格。GIFについて、透明色とdisposalを伴う全フレーム入力、異なる表示時間、有限／無限ループ、一時停止・速度・コマ送り、元/SR切替、全フレーム重ね処理、回転、保存、2フレーム目の失敗／中止、画像送り、メモリー見積りによる拒否を確認した。fake workerにも本番と同じ一時出力の上書き拒否を適用し、連続フレームのファイル再利用を検証する。

### GIF全フレーム保存・拡大時の位置保持（2026-09-22）

同じ3フレームGIFをX11・NCS×2＋NCS2×2で2倍→さらに2倍にし、2000×1120の全フレームをGIFへ保存した。qViewSRと独立したPillowデコーダーで、全3フレーム、100 / 200 / 300 ms、無限ループの保持を確認した。実機GUIシナリオは40.362秒で合格（起動・推論・保存・再生確認を含む機能試験値）。保存時の再推論はない。

拡大・パン後に実際のタイマーで60回以上フレーム更新し、変換行列と縦横スクロールバーの位置が変化しないことを確認した。修正前はSRフレーム更新120回で120画素の移動を再現した。自動試験では元GIF/SR、補間有無、0度/90度回転を組み合わせ、120回のフレーム更新と元/SRの20往復切替でも位置を保持する。
