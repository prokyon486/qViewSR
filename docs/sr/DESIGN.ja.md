...
- 倍率・ノイズ低減・重ね処理は全フレームに適用する。フレームをまたぐ推論や動き補正は行わない。保存は現在のフレームのPNG/JPEGのみで、通常の「フレームを保存」は表示中の元/SR、「SRフレームを保存」は常にSRをsRGBへ正規化して保存する。アニメーション書き出しは未実装。
...
初期実用版の対象はSDRのRGB/グレースケールJPEGとPNG。透明PNGはalphaを分離して保持する。この対象は上記の追加実装で拡張済み。HDR、16-bit精度でのSR、GIF以外の全フレームSR、学習、アニメーション書き出しは後続。対象外画像もqViewの通常閲覧は可能だが、正しい色の入力を保証できない形式ではSRを無効にする。16-bit PNGは原画像を保持し、SR時の8-bit化を表示する。
