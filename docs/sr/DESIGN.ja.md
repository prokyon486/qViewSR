...
- 状態表示は先頭行のみ・固定高さ。ラベルのsize hintは文字数に依存させず、操作ボタン後の残り幅を右端まで使用。全文はツールチップに保持。フルスクリーンでSRバーを非表示。ショートカットはメインウィンドウに残す。
- Qt標準の日本語とqView翻訳を実行ファイルへ埋め込み、未訳の設定項目を補完。初回更新時は日本語へ移行し、その後の明示的な言語選択は保持。
- decoderのQImageを共通入力とし、形式のJPEG/PNG限定を撤廃。WebPの元ICCも抽出。GIFは全フレームをSRし、フレーム間隔・ループ・alphaを保持して再生。GIF以外のアニメーションは現在のフレームを静止してSR、再開時に結果を破棄。CMYKは既にRGB化されたdecoder出力をsRGBと仮定する旨を表示。
...
- GUIで各回1.25〜4.00倍（既定4倍）を指定。worker・固定1032モデルの4倍契約は変えず、検証した4倍出力をGUIで指定寸法へSmoothTransformation縮小する。alphaを復元してpremultiplied形式で縮小し、透明境界の色に配慮する。各辺は入力寸法×指定倍率を整数画素へ丸める。
- 「元画像を超解像」は常にSourceFrame、「SRを重ねる」は現在保持中のSRのsRGBデータを入力とする。表示ICCや元ICCをSRへ再適用しない。元画像のQImageは不変。倍率・回数は新しい結果が正常に受理された時点で更新する。
- 正常完了まで前回のSRを保持し、中止・hash不一致・worker異常では置き換えない。画像送りでは世代を更新し、結果・倍率履歴を破棄する。GIFの再生再開は同じ結果を維持する。結果履歴のundo機能はなく、保持する完成SRは最新1枚（GIFは最新1組のフレーム列）。
- GUIの割当前見積りは今回の入力画素数×320 byte＋元画像・前回SRの実バイト数＋4デバイス分の予約768 MiB。追加縮小と前回結果の保持を含めた保守的な制限とする。指定倍率が小さくても4倍の中間出力が必要。workerの制限も維持する。
- 倍率変更や重ね処理では常駐workerを再初期化しない。Ctrl+Uは元画像から、Ctrl+Shift+UはSRからの開始。保存名と状態欄には累計倍率・処理回数を入れる。

### GIF全フレーム処理と再生

- `QImageReader::read()`を順に呼び、差分矩形・disposal・透明色を合成した原寸フレームをQtから取得する。各フレームの`nextImageDelay()`と`loopCount()`（0は1回、-1は無限）も保持する。デコードはQtConcurrentで実行し、途中の中止とファイルのsize/mtime変更を確認する。
- 1つのGUI jobが全フレームを所有する。workerへは1フレームずつ新しいjob IDで渡し、同じ常駐Sessionで処理する。各出力のhash・寸法・sRGB・ノイズ低減値を検証してメモリーへ読み込んだ後、一時出力を削除して次へ進む。中間PNGのディスク使用量は1フレーム分に抑える。
- 全フレームが成功した時だけ表示結果を交換する。途中の失敗・中止では前回の完成列を保持し、初回なら元のQMovieを継続する。画像送り・終了で世代を無効化し、フレーム列・表示キャッシュ・タイマーを解放する。
- 完成後は元のQMovieを静止し、Controllerのフレーム列と精密な単発QTimerで再生する。処理時間をフレームの待ち時間から引き、既存の再生速度（0〜1000%）を適用する。有限ループは最後のフレームで停止し、再開すると先頭へ戻る。手動での一時停止と自然終了を区別する。
- MainWindowの一時停止・コマ送り・速度変更・情報・フレーム保存をControllerへ接続する。元画像/SRの切替は同じフレーム位置で行い、ズームと回転を維持する。モニターICCごとの変換結果はフレーム単位で遅延キャッシュし、プロファイル変更時に破棄する。
- GIFのメモリー上限は独立設定`sr/animationMemoryMiB`（既定8192 MiB、範囲1024〜65536 MiB）。原寸全フレーム・出力全フレーム・表示キャッシュ・保持中の前回結果・最大1フレーム分の4倍中間処理・デバイス予約768 MiBを見積る。デコード前と推論開始前に検査し、静止画の従来上限も維持する。
- 倍率・ノイズ低減・重ね処理は全フレームに適用する。フレームをまたぐ推論や動き補正は行わない。「SRを保存」からGIF全フレームまたはPNG/JPEGの現在フレームを選択できる。通常の「フレームを保存」は表示中の元/SRをPNG/JPEGで保存する。

### GIF書き出しと表示位置の保持

- `src/sr/gif_export.*`でgiflib 5のストリーミングAPIを使用する。完成したsRGBフレーム列と回転角を共有参照で固定し、QtConcurrent/QPromiseでGUI外から出力する。デバイス再推論は不要。元の表示時間（10 ms単位）・有限/無限ループを保持し、画面側の再生速度は適用しない。
- 全フレームのRGBヒストグラムから共通パレットを作る。256色以内（透明色ありは255色以内）ならRGBを正確に保存する。それを超える場合はgiflibの減色と固定のordered ditherを使用し、色割当がフレームごとに変わらないようにする。ヒストグラムと標本のメモリーを制限し、フレーム変換は1枚ずつ行う。
- GIFの二値透明度に合わせalpha 128未満を透明へ変換。透明がある列では全画面フレーム＋背景へのdisposalにより、前フレームの絵柄が残らないようにする。グローバルパレット直後にsRGB ICCのApplication Extension（`ICCRGBG1012`）を埋め込む。ICC非対応リーダーでもパレット値はsRGB。
- `QSaveFile`へ書き、全フレームとGIF終端の正常書込後にcommitする。中止・書込失敗では既存ファイルを保持。元ファイルへの上書きを拒否し、保存中は新しいSR・設定変更・別の保存を無効化する。保存用フレームは画像送り・終了時の中止を確認して解放する。
- SRフレームの差替えで毎回`QRect::center()`→`centerOn()`を呼んでいたため、整数スクロールバーへの丸めが約1画素ずつ累積していた。同寸法では画像だけを交換して変換行列とスクロールバーを保持し、補間済み表示も同じ画素サイズで置換する。原寸とSRの寸法切替も表示幅と画像原点を保つため、整数スクロール位置をそのまま復元し、中心座標の往復変換を避ける。

参考: [giflib API](https://giflib.sourceforge.net/gif_lib.html)、[QGraphicsViewの中心座標と整数スクロールバー](https://doc.qt.io/qt-6/qgraphicsview.html#centerOn)、[QtConcurrentの進捗通知](https://doc.qt.io/qt-6/qtconcurrentrun.html#progress-reporting)。

### 画像送りの順序保持

- qViewから引き継いだ一覧更新は、毎回`QDir::Unsorted`で取得し直す一方、フォルダー・件数・設定が同じ場合はソートを省略していた。3秒以上表示してから移動するとこの経路を通り、名前順からOSの列挙順へ変わって現在位置も再計算される。先読みの有無や画像容量に依存せず再現する。
- ランダム以外は毎回、選択中の規則で並べ直す。日時・容量・種類が同じ場合は自然な名前順、照合結果も同じ場合は大文字小文字を区別した完全なファイル名を使う。同じ件数でも名前・日時・容量の変更を反映する。従来の各項目の昇順/降順の向きは維持する。
- ランダム順は同じフォルダー・設定での再走査時に既存の相対順を保つ。消えたファイルを除き、新しいファイルだけをシャッフルして末尾に加える。新しい一覧の構築後に表示中ファイルのindexを更新する。
...
- 実装上は役割を上記クラスに集約。後述の個別サービス名は将来の分割案。SR結果は現在画像のメモリー内に最新1件だけ保持する。GIFの表示変換済みフレームはメモリー内にキャッシュし、ファイルをまたぐ永続キャッシュは未実装。source keyは正立・sRGB化した入力PNGと元ICCのhash。読込前後のsize/mtime変更とキャッシュ登録前のrevisionを確認するが、ファイル全体hashによるrevision管理は後続。
...
| SR表示 | 明示操作で倍率指定／SRを重ねる。表示中の画像を保持して実行し、完成後切替 | キャンセル、画像切替、worker異常 |
| SR保存 | PNG/JPEG、sRGB ICC付き、別名保存 | 再読込、寸法、ICC、画面ICCの混入なし |
| ホスト環境 | Ubuntu 24.04、コンテナなし。旧runtimeは専用ディレクトリー | QtとOpenVINOの依存を別プロセスに隔離 |

初期実用版の対象はSDRのRGB/グレースケールJPEGとPNG。透明PNGはalphaを分離して保持する。この対象は上記の追加実装で拡張済み。HDR、16-bit精度でのSR、GIF以外の全フレームSR、学習、GIF以外のアニメーション書き出しは後続。対象外画像もqViewの通常閲覧は可能だが、正しい色の入力を保証できない形式ではSRを無効にする。16-bit PNGは原画像を保持し、SR時の8-bit化を表示する。
...
| 倍率 | モデル内部は×4固定。GUI側の出力は1.25〜4倍へ縮小可能（2倍専用モデルではない） |
