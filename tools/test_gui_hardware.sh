...
if [[ $# -ne 1 || ! -f "$1" ]]; then
    echo "Usage: $0 /path/to/test-image.png (static RGB JPEG/PNG)" >&2
    exit 1
fi
...
export QVIEWSR_GUI_ARTIFACTS="$qviewsr_repo/diagnostics/local/gui-check"
export QT_QPA_PLATFORM=${QT_QPA_PLATFORM:-offscreen}
exec "$qviewsr_repo/build/gui/tests/srtests" realHardwareGui
