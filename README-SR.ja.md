...
| sRGB ICC付きで別名保存 | 「SRを保存…」（GIFは「SRフレームを保存…」）／Ctrl+Shift+S |
...
GIFの元フレームと完成SRフレームはメモリーへ保持し、表示ICCの変換結果も再利用します。大きなGIFは処理前に必要量を見積り、設定上限を超える場合は案内します。デバイスはフレームごとに初期化せず、画像内のタイルを最大4本へ分配します。各フレームは独立に推論するため、動きに沿った時間方向の画質補正は行いません。保存は現在の1フレームのPNG/JPEGです。アニメーションGIFとしての書き出しには未対応です。
...
# GIF全フレームを2倍→さらに2倍にし、再生・コマ送り・保存を確認
